...
data BinaryList a = Nil | Zero (BinaryList (a, a)) | One a (BinaryList (a, a)) deriving Show

uncons:: BinaryList a -> (a, BinaryList a)
uncons Nil = error "empty list "
uncons (One x Nil) = (x, Nil)
uncons (One x ps) = (x, Zero ps)
uncons (Zero ps) = let ((x, y), psf) = uncons ps in (x, One y psf)
...
instance RandomAccessList BinaryList where
  ...
  -- update 0 e (One x ps) = One e ps
  -- update i e (One x ps) = cons x (update (i - 1) e (Zero ps))
  -- update i e (Zero ps) = Zero (update (i `div` 2) p ps)
    -- where
